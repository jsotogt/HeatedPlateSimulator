...
public class Twfahp implements HeatedPlateSimulator {
    ...
    private float unchangedError; //the amount of error for the differences between the plates to stop loop
    Float oldPlate[][]; 
    Float newPlate[][]; 
    ...
    private boolean done(int d) {
        ...
        if ((Math.abs(middleNew - middleOld) < unchangedError) && (count > (d*d/2))){
            return true;
        }        
        else return false;
    }
